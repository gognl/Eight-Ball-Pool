; V3a - THIS VERSION CONTAINS:
		; - BB Collision Detection
		; - Friction with table
		; - WASD movement
		; - BW Collision Detection & Response
		; - When 2 balls collide, they do not enter each other
		; Some bugs: sometimes balls leave trails when colliding, missing collision, etc.
...
STACK 100h
DATASEG
Balls dw 17*1, 17+128, 6 dup(0), 12, 7 dup(?) 
	  dw 17*27, 17*6+128, 6 dup(0), 12, 7 dup(?) 
	  dw 17*40, 17*20+128, 6 dup(0), 12, 7 dup(?) 
	  dw 17*11, 17+128, 6 dup(0), 12, 7 dup(?) 
	  dw 17*6, 17*15+128, 6 dup(0), 12, 7 dup(?) 
	  dw 17*15, 17*6+128, 6 dup(0), 12, 7 dup(?) 
	  dw 17*47, 17*7+128, 6 dup(0), 12, 7 dup(?) 
	  dw 17*34, 17*10+128, 6 dup(0), 12, 7 dup(?) 
	  dw 17*47, 17*15+128, 6 dup(0), 12, 7 dup(?) 
	  dw 17*50, 17+128, 6 dup(0), 12, 7 dup(?)
...
; OUTPUT: [bp+6]+[bp+8] 32bit float number
;
; TODO: Use https://en.wikipedia.org/wiki/Fast_inverse_square_root instead
proc magnitude
	push bp
	mov bp, sp
	push eax
...
; INPUT: [bp+4] y0, [bp+6] x0, [bp+8] FPUmem, [bp+10] JUNK
