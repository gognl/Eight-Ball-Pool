; V10b - THIS VERSION INCLUDES:
		; - Collision physics - fixed bug with v=0 balls, and made the collision-radius slightly bigger.
		
		; THE FULL GAME, WITHOUT THE GOOD GRAPHICS (DURING THE GAME AND BEFORE AND AFTER THE GAME)
...
balls dw 272, 384, 8 dup(0), 40h, ?, 1, 3 dup(?)
	  ...
	  dw 10 dup(0), 1dh, ?, 1, 3 dup(?)
	  dw 10 dup(0), 1bh, ?, 1, 3 dup(?)
	  dw 10 dup(0), 19h, ?, 1, 3 dup(?)
	  dw 10 dup(0), 17h, ?, 1, 3 dup(?)
	  dw 10 dup(0), 15h, ?, 1, 3 dup(?)
	  dw 10 dup(0), 13h, ?, 1, 3 dup(?)
	  dw 10 dup(0), 11h, ?, 1, 3 dup(?)
...
; INPUT: [bp+34] always_check_mask, [bp+32] temporary_previous_balls, [bp+30] nine_pocketed, [bp+28] pocketed, [bp+26] rail, [bp+24] cue_collided, [bp+22] foul, [bp+20] previous_balls, [bp+18] balls, [bp+16] friction, [bp+14] gamemode, [bp+12] fpu_mem, [bp+10] grid, [bp+8] ball_template, [bp+6] cursor_backup, [bp+4] previous_cursor
; OUTPUT: None.
