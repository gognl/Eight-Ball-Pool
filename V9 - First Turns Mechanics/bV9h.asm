; V9h - THIS VERSION INCLUDES:
		; - Collision physics
		; - Game
		; - Initial racking - diamond shaped, random
		; - Break fouls - FIXED
		; - Cue in hand behind head string on break shot
		; - Push out (temporary keyboard button p) - including an option to choose whether to stay or return the turn.
...
STACK 100h
DATASEG
balls dw 272, 384, 8 dup(0), 40h, ?, 1, 3 dup(?)
	  dw 752, 384, 8 dup(0), 1fh, ?, 1, 3 dup(?)
	  dw 752+19, 384-11, 8 dup(0), 1dh, ?, 1, 3 dup(?)
	  dw 752+19, 384+11, 8 dup(0), 1bh, ?, 1, 3 dup(?)
	  dw 752+19+19, 384-22, 8 dup(0), 19h, ?, 1, 3 dup(?)
	  dw 752+19+19, 384+22, 8 dup(0), 17h, ?, 1, 3 dup(?)
	  dw 752+19+19+19, 384-11, 8 dup(0), 15h, ?, 1, 3 dup(?)
	  dw 752+19+19+19, 384+11, 8 dup(0), 13h, ?, 1, 3 dup(?)
	  dw 752+19+19+19+19, 384, 8 dup(0), 11h, ?, 1, 3 dup(?)
	  dw 752+19+19, 384, 8 dup(0), 0bh, ?, 1, 3 dup(?)
	  ; 640 bytes
...
proc canPutBallBehindHeadString
	...
	;;;;;;;;
		...
		mov cx, 9
		can_put_ball_loop_head:
			mov di, cx
			shl di, 5
			add di, [bp+6]
			cmp [word ptr di+24], 0
			je collision_not_detected_in_hand_b_head ; ball doesnt exist
			...
			fnstsw ax ; moves the FPU flag register to ax
			sahf ; moves ax to the flag register
			jnae collision_not_detected_in_hand_b_head ; if r1+r2>=mag
				mov ax, 1
				mov [bp+10], ax ; returns 1
				jmp finished_checking_can_put_ball_head
			collision_not_detected_in_hand_b_head:
			dec cx
			cmp cx, 0
			jge can_put_ball_loop_head
...
start:
	...
	cmp [byte ptr di], 1
	je second_shot ; legal break
	mov di, offset foul
	cmp [byte ptr di], 0
	jne second_shot ; a foul was committed anyway
	...
	second_shot:
		...
		push offset current_player
		call handleFouls
		; ADD GAMEMODE CHECK HERE, MAYBE NEED IN HAND
		
		mov di, offset gamemode
		cmp [byte ptr di], 1
		je second_gamemode_1_WaitingForCueBegin
		...
		second_gamemode_1_WaitingForCueBegin:
		mov di, offset previous_cursor ; previous_cursor offset
		push offset cursor_backup
		push [word ptr di]
		push [word ptr di+2]
		call backupBehindCursor
		
		gamemode1_waiting_for_click_p:
			mov ah, 1
			int 16h
			jz buffer_empty_p
			xor ah, ah
			...
			push cx
			push dx
			push offset cursor_backup; cursor_Backup
			push offset previous_cursor ; previous_cursor
			call redrawCursor
				
			shr bx, 1
			jnc gamemode1_waiting_for_click_p
		...
		waiting_for_force_p:
			...
			shr bx, 1
			jc waiting_for_force_p
		
		;;;;;;;;;;;;;; After release ;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;
		mov di, offset previous_cursor ; previous_cursor
		push offset cursor_backup ; cursor_backup
		push [word ptr di]
		push [word ptr di+2]
		call hideCursor
		...
		jne cursor_moved_p
			...
			push dx
			call drawCursor
			jmp gamemode1_waiting_for_click_p
		cursor_moved_p:
		; checks if the cursor even moved
					
		push offset balls ; balls offset
		push offset fpu_mem ; fpu_mem
		push ax
		push bx
		push cx
		push dx
		...
		mov di, offset balls ; balls offset
					
		pop [dword ptr di+12] ; acc_x
		pop [dword ptr di+16] ; acc_y
		
		mov di, offset gamemode ; gamemode offset
		mov [byte ptr di], 0
