...
foul db 0 ; 1 if a foul was commited, else 0
		...
		collision_bb_loop:
			...
			je not_this_ball_collision_bb
					...
					je not_this_ball_collision_bb
						...
						cmp [word ptr bp+8], 0
						jne no_foul_commited_bad_hit
							mov di, [bp+18] ; cue_collided offset
							cmp [byte ptr di], 1
							je no_foul_commited_bad_hit
								mov [byte ptr di], 1
								...
								pop di
								cmp di, bx
								je no_foul_commited_bad_hit ; no foul commited
								
								mov di, [word ptr bp+16] ; foul offset
								mov [byte ptr di], 1

						no_foul_commited_bad_hit:
...
proc pockets
	...
	;;;;;;;;;;
		...
		je didnt_enter_any_pocket
			...
			cmp [word ptr bp+4], 0
			jne no_foul_commited_scratch
				mov di, [word ptr bp+16] ; foul offset
				mov [byte ptr di], 1
			no_foul_commited_scratch:
...
start:
	...
	main_loop:
		...
		gamemode_1_WaitingForCue:
			...
			cmp [byte ptr di], al
			je no_foul_commited_no_rail
				mov di, offset rail
				cmp [word ptr di], ax
				jne no_foul_commited_no_rail
					mov di, offset pocketed
					cmp [byte ptr di], al
					jne no_foul_commited_no_rail
						mov di, offset foul
						mov [byte ptr di], 1
			no_foul_commited_no_rail:
