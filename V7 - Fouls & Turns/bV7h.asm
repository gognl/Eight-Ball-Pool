; V7h - THIS VERSION INCLUDES:
		; - Collision physics
		; - Pockets
		; - Cue
		; - Correct balls colors
		; - Fouls - bad hit, no rail, scratch, miss
		; - Turns (2 players) - player changed when a foul is committed 
		; - Organized fouls and turns (divided into procedures)
		; - If a player has 3 consecutive fouls, the game ends.
...
foul db 0 ; 1 if a foul was committed, else 0
...
; INPUT: [bp+16] foul offset, [bp+14] pocketed offset, [bp+12] grid offset, [bp+10] previous_balls offset, [bp+8] Balls_template offset, [bp+6] balls offset, [bp+4] ball index
; OUTPUT: [bp+16] 0 if the ball entered a pocket, 1 if not.
proc pockets
	push bp
	mov bp, sp
	push di bx
...
; INPUT: [bp+4] balls offset, [bp+6] Balls_template offset, [bp+8] previous_balls offset, [bp+10] grid offset, [bp+12] fpu_mem offset, [bp+14] gamemode offset, [bp+16] foul offset, [bp+18] cue_collided offset, [bp+20] rail offset, [bp+22] pocketed offset
; OUTPUT: None.
...
; INPUT: [bp+14] fouls_counter offset, [bp+12] cue_collided offset, [bp+10] rail offset, [bp+8] pocketed offset, [bp+6] foul offset, [bp+4] current_player offset
; OUTPUT: None.
proc handleFouls
	push bp
	mov bp, sp
	...
	;;;;;;;;;;
		...
		jne no_miss
			...
			jne miss_and_foul
				...
				add di, ax
				dec [byte ptr di] ; decreases the foul counter so the miss won't be counted as a foul.
				xor al, al
			miss_and_foul:
			mov di, [bp+6] ; foul offset
			mov [byte ptr di], 1
...
; INPUT: [bp+24] pocketed offset, [bp+22] rail offset, [bp+20] cue_collided offset, [bp+18] foul offset, [bp+16] previous_balls offset, [bp+14] balls offset, [bp+12] friction offset, [bp+10] gamemode offset, [bp+8] fpu_mem offset, [bp+6] grid offset, [bp+4] ball_template offset
; OUTPUT: None.
