; V5b - THIS VERSION INCLUDES:
		; - Collision physics
		; - Pockets: balls are deleted when they enter a pocket
...
; INPUT: [bp+14] Balls_template offset, [bp+12] FPUmem offset, [bp+10] Balls offset, [bp+8] ball index, [bp+6] Grid offset, [bp+4] BallsPrevious offset
; OUTPUT: None.
proc collision_bb
	push bp
	mov bp, sp
...
; INPUT: [bp+12] Grid offset, [bp+10] BallsPrevious offset, [bp+8] Balls_template offset, [bp+6] Balls offset, [bp+4] ball index
; OUTPUT: [bp+6] 0 if the balled entered a pocket, 1 if not.
proc pockets
	push bp
	mov bp, sp
	push di ax
	;;;;;;;;;;
		...
		je didnt_enter_any_pocket
			...
			shl di, 5
			add di, [bp+6]
			add di, 20 ; di = address of existence property of the ball
			xor ax, ax
			mov [di], ax ; doesnt exist anymore LOL
			mov [bp+12], ax
...
; Draws a pocket
; INPUT: [bp+4] Pocket number (0/1/2/3/4/5)
; OUTPUT: None.
proc draw_pocket
	push bp
	mov bp, sp
