; V6c - THIS VERSION INCLUDES:
		; - Collision physics
		; - Pockets
		; - Cue - with potentially a bug
...
; INPUT: [bp+12] FPUmem offset, [bp+10] direction x (not normalized), [bp+8] direction y (not normalized), [bp+6] released x, [bp+4] released y
; OUTPUT: [bp+6]+[bp+8] acc_x, [bp+10]+[bp+12] acc_y
proc cue
	push bp
	mov bp, sp
	push bx cx dx edi esi
	;;;;;;;;;;
		; acc=Unit(v1)/(inv_mag(v1-v2)*const)
		mov bx, [bp+12] ; FPUmem
		...
		fld [dword ptr bx]
		fdiv ST(0), ST(1)
		fstp [dword ptr bp+6] ; acc_x
		
		mov [bx], esi
		fld [dword ptr bx]
		fdiv ST(0), ST(1)
		fstp [dword ptr bp+10] ; acc_y
		
	;;;;;;;;;;
	pop esi edi dx cx bx bp
	ret 2
	endp cue
...
; INPUT: [bp+4] Balls offset, [bp+6] Balls_template offset, [bp+8] BallsPrevious offset, [bp+10] Grid offset, [bp+12] FPUmem offset, [bp+14] PocketsHit offset, [bp+16] Gamemode offset
; OUTPUT: None.
